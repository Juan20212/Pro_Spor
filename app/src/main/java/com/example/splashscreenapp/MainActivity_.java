...
public class MainActivity_ extends AppCompatActivity implements RecyclerAdapter.RecyclerItemClick, SearchView.OnQueryTextListener {
    ...
    private void getItemSQL(){
        retrofilApiService.getItemsDB().enqueue(new Callback<List<ItemList>>() {
            ...
            public void onResponse(Call<List<ItemList>> call, Response<List<ItemList>> response) {
                items = response.body();
                adapter = new RecyclerAdapter(items, MainActivity_.this);
                rvLista.setAdapter(adapter);
            }

            @Override
            public void onFailure(Call<List<ItemList>> call, Throwable t) {
                Toast.makeText(MainActivity_.this, "Error"+t.getMessage(), Toast.LENGTH_SHORT).show();
